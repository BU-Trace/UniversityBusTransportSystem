...
export default function DriverDashboard() {
  ...
  const handleStop = () => {
    if (watchId.current !== null) navigator.geolocation.clearWatch(watchId.current);
    setStatus('idle'); setLocation(null); setSidebarOpen(true);
  };
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-gray-50 font-sans">
      ...
      `}>
        <button onClick={() => setSidebarOpen(false)} className="absolute top-4 right-4 text-white/70 hover:text-white text-2xl font-light">✕</button>

        <div className="flex flex-col h-full overflow-hidden">
          {/* Brand Logo */}
          <div className="flex items-center mb-10">
            <div className="bg-white rounded-full w-12 h-12 flex items-center justify-center">
              <Image src="/static/BUTracelogo-modified.png" alt="Logo" width={40} height={40} priority />
            </div>
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h2 className="mt-4 text-xl font-bold uppercase text-center leading-tight">{driver.name}</h2>
            <p className="text-red-200 text-[10px] font-medium uppercase tracking-widest mt-1 opacity-80">Employee ID: {driver.id}</p>
          </div>

          {/* Static Data from "Database" */}
          <div className="flex-1 space-y-4">
            <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
              <span className="text-[10px] uppercase font-black text-red-200 block mb-1">Assigned Bus</span>
              <p className="text-base font-bold">{driver.busNo}</p>
              <p className="text-[10px] opacity-60">{driver.reg}</p>
            </div>
            <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
              <span className="text-[10px] uppercase font-black text-red-200 block mb-1">Current Route</span>
              <p className="text-sm font-semibold leading-tight">{driver.route}</p>
            </div>
            <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
              <span className="text-[10px] uppercase font-black text-red-200 block mb-1">Duty Status</span>
        ...
        <div className="absolute inset-0 z-0">
          {location ? <BusMap location={location} /> : (
            <div className="h-full w-full flex flex-col items-center justify-center bg-slate-50">
              ...
              <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">System Ready</h3>
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 flex gap-3 z-1001 w-[92%] max-w-md">
          {status === 'idle' ? (
            ...
            <>
              ...
              <button onClick={handleStop} className="flex-1 bg-red-600 text-white py-5 rounded-4xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition-transform">End Shift</button>
            </>
          )}
        </div>
      </main>
